...
## Corpus Check
- Corpus is ~13,835 words - fits in a single context window. You may not need a graph.

## Summary
- 338 nodes · 588 edges · 28 communities (23 shown, 5 thin omitted)
- Extraction: 83% EXTRACTED · 17% INFERRED · 0% AMBIGUOUS · INFERRED: 98 edges (avg confidence: 0.7)
- Token cost: 0 input · 79,745 output
...
1. `ForwardFillInator` - 23 edges
2. `SwapRateETL` - 20 edges
3. `SwapRateRecord` - 20 edges
4. `EconomicCalendarEventRecord` - 20 edges
5. `CandlestickRecord` - 19 edges
6. `PositioningBucketRecord` - 19 edges
7. `CandlestickETL` - 17 edges
8. `PositioningETL` - 17 edges
...
- `EconomicCalendarETL` --references--> `Finnhub API`  [EXTRACTED]
  src/forex/etl/EconomicCalendarETL.py → README.md
...
- **Shared Pydantic Record Schema Pattern (MEASUREMENT/TAGS/FIELDS/to_influx_dict)** — etl_models_candlestickrecord, etl_models_forwardfilledcandlestickrecord, etl_models_swapraterecord, etl_models_economiccalendareventrecord, etl_models_positioningbucketrecord [EXTRACTED 1.00]
- **Oanda-Sourced ETL Classes** — etl_candlesticketl_candlesticketl, etl_swaprateetl_swaprateetl, etl_positioningetl_positioningetl, concept_oanda_rest_api [EXTRACTED 1.00]
- **Dormant Pipelines (Cost/Access-Blocked, Not Broken)** — flows_economic_calendar_flow_economic_calendar_flow, flows_positioning_flow_positioning_flow, etl_economiccalendaretl_economiccalendaretl, etl_positioningetl_positioningetl, readme_economic_calendar_pipeline, readme_positioning_pipeline [EXTRACTED 1.00]

## Communities (28 total, 5 thin omitted)

### Community 0 - "Data Model & Schema Layer"
Cohesion: 0.07
Nodes (20): BaseModel, forex-ML (downstream project), forex-strategy (downstream project), InfluxDB, CandlestickRecord, EconomicCalendarEventRecord, ForwardFilledCandlestickRecord, PositioningBucketRecord (+12 more)
...
Cohesion: 0.09
Nodes (29): candlestick_batch_flow(), candlestick_flow(), check_market_open_task(), fetch_candlestick_data(), insert_to_influxdb(), _make_ifc(), Prefect flows: fetch Oanda candlesticks → InfluxDB.  Single instrument (ad-hoc):, Run candlestick_flow for each instrument sequentially. (+21 more)

### Community 3 - "Economic Calendar ETL (Dormant)"
Cohesion: 0.11
Nodes (19): Dormant Pipelines Policy, Finnhub API, EconomicCalendarETL, Pure transform, kept separate from any HTTP call so it's directly testable     a, Pulls scheduled economic calendar events (release time, country, impact,     act, _records_from_calendar_response(), economic_calendar_flow(), fetch_economic_calendar() (+11 more)
...
Cohesion: 0.18
Nodes (5): is_market_open(), is_market_open_at_time(), _at(), TestMarketHours, TestTimezoneObject

### Community 6 - "Forward-Fill DST Logic"
Cohesion: 0.13
Nodes (6): DST-Aware Grid Logic Gotcha, ndarray, ForwardFillInator, # TODO: replace with an explicit holiday calendar to also drop/flag extended, Every timestamp a candle is expected to exist at, from `mn` to `mx`.          Bu, DST-Aware Expected-Bar Grid Fix

### Community 7 - "Positioning ETL (Dormant)"
Cohesion: 0.15
Nodes (12): Oanda REST API, PositioningETL, Pure transform, kept separate from any HTTP call so it's directly testable     a, Pulls OANDA's per-instrument order-book and position-book snapshots (the     lat, _records_from_book_response(), Positioning-Bucket Pipeline (blocked), test_compute_positioning_fetches_both_books_for_every_instrument(), test_fit_produces_valid_influxdb_dicts() (+4 more)

### Community 9 - "Forward-Fill DST Tests"
Cohesion: 0.27
Nodes (11): DataFrame, datetime, _make_hourly_frame(), _make_local_time_frame(), Regression coverage for a real, confirmed-in-production bug: H4/D candles     ar, This is exactly what the pre-fix call order (account_for_holiday_market_, _run_full_fit(), _run_through_forward_fill() (+3 more)

### Community 10 - "Positioning Flow & InfluxDB Tool"
Cohesion: 0.14
Nodes (8): fetch_positioning(), insert_positioning_to_influxdb(), _make_ifc(), positioning_flow(), Prefect flow: fetch OANDA per-instrument order-book/position-book snapshots → In, No market-hours gate -- OANDA continues to serve its most recent order-book/, Series, InfluxDbTool
...
- **Why does `ForwardFillInator` connect `Forward-Fill DST Logic` to `Data Model & Schema Layer`, `Forward-Fill DST Tests`, `Candlestick & Forward-Fill Flows`?**
  _High betweenness centrality (0.167) - this node is a cross-community bridge._
- **Why does `SwapRateETL` connect `Swap-Rate ETL` to `Data Model & Schema Layer`, `Candlestick & Forward-Fill Flows`, `Positioning ETL (Dormant)`?**
  _High betweenness centrality (0.159) - this node is a cross-community bridge._
- **Why does `InfluxDB` connect `Data Model & Schema Layer` to `Candlestick ETL`, `Repo Conventions & Refactor Rationale`, `Forward-Fill DST Logic`?**
